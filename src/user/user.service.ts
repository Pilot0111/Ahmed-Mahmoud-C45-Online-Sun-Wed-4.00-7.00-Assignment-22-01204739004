import { ConflictException, Injectable, HttpException, HttpStatus, NotFoundException, UnauthorizedException, ForbiddenException, BadRequestException } from '@nestjs/common';
...
import { comparePassword, hashPassword } from 'src/common/utils/security/hash.security';
...
export class UserService {
  ...
  async signUp(body: CreateUserDto) {
    const { firstName, lastName, email, password, address, phone, age, gender } = body;
    const normalizedEmail = email.toLowerCase();

    const existingUser = await this.userRepository.findOne({ filter: { email: normalizedEmail } });
    if (existingUser) {
  ...
  async resendOtp(body: ResendOtpDto) {
    ...
    const user = await this.userRepository.findOne({ filter: { email: normalizedEmail } });
  ...
  async confirmEmail(body: ConfirmEmailDto) {
    ...
    const otpKey = this.redisService.generateOtpKey({ email: normalizedEmail, subject: EventEnum.confirmEmail });
    ...
    const user = await this.userRepository.findOne({
      filter: { email: normalizedEmail, confirmed: { $ne: true }, provider: providerEnum.system },
    });
    ...
    await this.redisService.deleteKey(this.redisService.maxOtpTriesKey(normalizedEmail));
  ...
  async login(body: LoginDto) {
    ...
    if (isBlocked !== undefined && isBlocked > 0) {
      throw new ForbiddenException(`Account temporarily banned. Please try again after ${isBlocked} seconds`);
    }
    ...
    if (!match) {
      ...
      if (parseInt(failed_tries) >= 5) {
        ...
        throw new ForbiddenException("Account temporarily banned due to 5 consecutive failed login attempts");
      }
    ...
    await this.redisService.deleteKey(this.redisService.maxLoginTriesKey(normalizedEmail));
  ...
  async signUpGmail(body: SignInGmailDto) {
    ...
    const { email: googleEmail, email_verified, name, picture, given_name, family_name } = payload as TokenPayload;
    if (!googleEmail) throw new BadRequestException("Email not found in Google token");

    const finalEmail = googleEmail.toLowerCase();
    let user = await this.userRepository.findOne({ filter: { email: finalEmail } });
    ...
    if (user.provider === providerEnum.system) {
      throw new BadRequestException("Please login with your email and password");
    }
  ...
  async resetPassword(body: ResetPasswordDto) {
    ...
    if (!otpValue) throw new NotFoundException("Reset code expired or not found");
  ...
  async updatePassword(user: any, body: UpdatePasswordDto) {
    ...
    const fullUser = await this.userRepository.findOne({ filter: { _id: user._id } });
  ...
  async getProfilePicPresignedUrl(user: any, body: PresignedUrlDto) {
    ...
    return { message: "Profile picture presigned URL generated successfully", data: result };
  }

  async getFile(user: any, keyParam: string | string[], isDownload: boolean, res: Response) {
  ...
  async getPresignedUrlByKey(user: any, keyParam: string | string[], isDownload: boolean) {
    let key = Array.isArray(keyParam) ? keyParam.join('/') : keyParam;
    if (key) key = key.replace(/^\/+/, "");
  ...
  async deleteFolder(user: any, folderPathParam: string | string[]) {
    let folderPath = Array.isArray(folderPathParam) ? folderPathParam.join('/') : folderPathParam;
    ...
    return { message: `Successfully deleted folder '${folderPath}' and its ${keys.length} files.` };
  }
  ...
  async sendNotification(body: SendNotificationDto) {
    ...
    return { message: "Notification sent successfully", data: { messageId: response } };
  }
  ...
  update(id: number, updateUserDto: any) {
    return { message: `This action updates a #${id} user with data: ${JSON.stringify(updateUserDto)}` };
  }
